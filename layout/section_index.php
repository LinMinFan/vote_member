...
$topsql =
    "
    SELECT `vote_member_subjects` . * , A.`參加人數`
    FROM `vote_member_subjects`,(SELECT * ,COUNT(DISTINCT `user_id`) as '參加人數' FROM `vote_member_log`  GROUP BY `subject_id`
    )A
    WHERE `vote_member_subjects`.`id` = A.`subject_id`
    ORDER BY  A.`參加人數` DESC,`vote_member_subjects` . `end` DESC
    LIMIT 4
";
